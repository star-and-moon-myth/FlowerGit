package com.lnsf.utils.consoletable;

import java.util.ArrayList;
import java.util.List;

import com.lnsf.utils.consoletable.enums.Align;
import com.lnsf.utils.consoletable.enums.NullPolicy;
import com.lnsf.utils.consoletable.table.Body;
import com.lnsf.utils.consoletable.table.Cell;
import com.lnsf.utils.consoletable.table.Header;
import com.lnsf.utils.consoletable.util.StringPadUtil;
import org.apache.commons.lang.StringUtils;

/**
 * Create by IntelliJ IDEA console table
 * 
 * @author chenlei
...
public class ConsoleTable {
	...
	List<String> getLines() {
		...
		if ((header != null && !header.isEmpty())
			...
			lines.addAll(header.print(columnWidths, horizontalSep, verticalSep,
					joinSep));
			lines.addAll(body.print(columnWidths, horizontalSep, verticalSep,
					joinSep));
		}
		return lines;
	}
	...
	public static class ConsoleTableBuilder {
		...
		public ConsoleTable build() {
			...
			if (!consoleTable.header.isEmpty() || !consoleTable.body.isEmpty()) {
				...
				int maxColumn = allRows.stream().map(List::size)
						.mapToInt(size -> size).max().getAsInt();
				int minColumn = allRows.stream().map(List::size)
						.mapToInt(size -> size).min().getAsInt();
	...
	public static void main(String[] args) {
				...
				add(new ArrayList<Cell>() {
					{
						...
						add(new Cell("9876543中文测试21000000"));
					}
				});
				add(new ArrayList<Cell>() {
					{
		...
		// default
		new ConsoleTable.ConsoleTableBuilder().addHeaders(header).addRows(body)
				.build().print();
